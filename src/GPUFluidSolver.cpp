...
GPUFluidSolver::GPUFluidSolver(int nx, int ny, int nz) : NX(nx), NY(ny), NZ(nz) {
    size = NX * NY * NZ;
    // REFACTOR: New default bounds to match the scale
    gridMin[0] = -15.0f; gridMin[1] = -10.0f; gridMin[2] = -10.0f;
    gridMax[0] = 15.0f; gridMax[1] = 10.0f; gridMax[2] = 10.0f;
...
void GPUFluidSolver::initBuffers() {
    ...
    glGenBuffers(1, &f_in_SSBO); glBindBuffer(GL_SHADER_STORAGE_BUFFER, f_in_SSBO); glBufferData(GL_SHADER_STORAGE_BUFFER, initial_f.size() * 4, initial_f.data(), GL_DYNAMIC_DRAW);
    glGenBuffers(1, &f_out_SSBO); glBindBuffer(GL_SHADER_STORAGE_BUFFER, f_out_SSBO); glBufferData(GL_SHADER_STORAGE_BUFFER, initial_f.size() * 4, initial_f.data(), GL_DYNAMIC_DRAW);
    ...
    glGenTextures(1, &solidTexture); glBindTexture(GL_TEXTURE_3D, solidTexture); glTexImage3D(GL_TEXTURE_3D, 0, GL_R8UI, NX, NY, NZ, 0, GL_RED_INTEGER, GL_UNSIGNED_BYTE, nullptr);
    ...
    glGenBuffers(1, &wakeCandidateSSBO); glBindBuffer(GL_SHADER_STORAGE_BUFFER, wakeCandidateSSBO); glBufferData(GL_SHADER_STORAGE_BUFFER, 32768, nullptr, GL_DYNAMIC_COPY);
    ...
    glGenBuffers(1, &wakeSSBO); glBindBuffer(GL_SHADER_STORAGE_BUFFER, wakeSSBO); glBufferData(GL_SHADER_STORAGE_BUFFER, 49152, nullptr, GL_DYNAMIC_DRAW);
...
void GPUFluidSolver::setGridBounds(M3DVector3f min, M3DVector3f max, M3DVector3f localWind) {
    ...
    if (ox != 0 || oy != 0 || oz != 0) { dispatchShift(ox, oy, oz, localWind); prevGridMin[0] += (float)ox*dx[0]; prevGridMin[1] += (float)oy*dx[1]; prevGridMin[2] += (float)oz*dx[2]; }
...
void GPUFluidSolver::dispatchShift(int ox, int oy, int oz, M3DVector3f localWind) {
    if (!shiftShader) return; shiftShader->use();
...
void GPUFluidSolver::step(float dt, M3DVector3f planeVel, M3DMatrix44f planeWaxis, M3DVector3f planePos, float simTime) {
    ...
    dispatchWakeInject(planePos); dispatchCollision();
    if (streamShader) { streamShader->use(); streamShader->setUniform("u_wind", UNI_VEC_3, localWindLattice); dispatchStream(); }
    dispatchReconstruct(); glMemoryBarrier(GL_ALL_BARRIER_BITS);
    dispatchParticleAdvect(dt, planePos, planeWaxis, simTime); glMemoryBarrier(GL_ALL_BARRIER_BITS);
    dispatchForceCompute(); dispatchWakeExtract(dt, planeWaxis, planePos);
    if (stabilityShader) {
        ...
        if (has_error) {
            ...
            for (int i=0; i<19; i++) for (int j=0; j<size; j++) initial_f[i*size+j] = w[i];
            glBindBuffer(GL_SHADER_STORAGE_BUFFER, f_in_SSBO); glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, initial_f.size()*4, initial_f.data());
            glBindBuffer(GL_SHADER_STORAGE_BUFFER, f_out_SSBO); glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, initial_f.size()*4, initial_f.data());
...
void GPUFluidSolver::debugUniformVelocity(M3DVector3f vel) {
    ...
    debugUniformVelShader->use(); glBindImageTexture(1, velocityTexture, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);
...
void GPUFluidSolver::resetParticles() {
    ...
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, particleSSBO); glBufferData(GL_SHADER_STORAGE_BUFFER, initial_particles.size()*32, initial_particles.data(), GL_DYNAMIC_DRAW);
}

void GPUFluidSolver::dispatchCollision() {
    if (!collisionShader) return; collisionShader->use();
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, f_in_SSBO); glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, f_out_SSBO);
...
void GPUFluidSolver::dispatchReconstruct() {
    if (!reconstructShader) return; reconstructShader->use();
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, f_in_SSBO); glBindImageTexture(1, velocityTexture, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);
...
void GPUFluidSolver::dispatchParticleAdvect(float dt, M3DVector3f planePos_arg, M3DMatrix44f planeWaxis, float simTime) {
    if (!particleAdvectShader) return; particleAdvectShader->use();
    ...
    glDispatchCompute(numParticles / 256 + 1, 1, 1); glMemoryBarrier(GL_ALL_BARRIER_BITS);
}

void GPUFluidSolver::dispatchForceCompute() {
    if (!forceComputeShader) return; forceComputeShader->use();
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, f_in_SSBO); glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, f_out_SSBO);
    glBindImageTexture(3, solidTexture, 0, GL_FALSE, 0, GL_READ_ONLY, GL_R8UI);
    ...
    float scale_val = 0.001f, alpha = 0.08f;
...
void GPUFluidSolver::dispatchWakeExtract(float dt, M3DMatrix44f planeWaxis, M3DVector3f planePos) {
    if (!wakeExtractShader) return; wakeExtractShader->use();
...
void GPUFluidSolver::dispatchWakeInject(M3DVector3f planePos) {
    if (!wakeInjectShader) return; const std::list<WakeVortex>& vortices = wakeManager.getVortices(); if (vortices.empty()) return;
    ...
    wakeInjectShader->use(); glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, wakeSSBO); glBindImageTexture(1, velocityTexture, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA16F);
...
void GPUFluidSolver::drawParticles(M3DMatrix44f mvp) {
    ...
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, particleSSBO);
    particleRenderShader->setUniform("modelViewProj", UNI_MATRIX_4, mvp);
...
void GPUFluidSolver::clearSolid() {
    GLuint zero = 0; glBindTexture(GL_TEXTURE_3D, solidTexture); glClearTexImage(solidTexture, 0, GL_RED_INTEGER, GL_UNSIGNED_BYTE, &zero);
}

void GPUFluidSolver::voxelizePart(ObjModel* model, M3DMatrix44f worldTransform) {
    if (!voxelizeShader || !model) return;
    GLuint triSSBO = model->getTriSSBO(); glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, triSSBO);
    voxelizeShader->use(); glBindImageTexture(1, solidTexture, 0, GL_FALSE, 0, GL_READ_WRITE, GL_R8UI);
    voxelizeShader->setUniform("gridMin", UNI_VEC_3, gridMin); voxelizeShader->setUniform("gridMax", UNI_VEC_3, gridMax);
    unsigned int nTri = (unsigned int)(model->getNumIndices() / 3); voxelizeShader->setUniform("numTriangles", UNI_INT_1, &nTri);
...
void GPUFluidSolver::voxelizeCylinder(M3DVector3f center, float radius, float height, int axis) {
    ...
    voxelizeCylinderShader->use(); glBindImageTexture(1, solidTexture, 0, GL_FALSE, 0, GL_READ_WRITE, GL_R8UI);
...
void GPUFluidSolver::voxelizeGround(float groundZ) {
    ...
    voxelizeGroundShader->use(); glBindImageTexture(1, solidTexture, 0, GL_FALSE, 0, GL_READ_WRITE, GL_R8UI);
