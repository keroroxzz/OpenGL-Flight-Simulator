...
ObjModel::ObjModel(const char* path,
                   ...
                   float scale_y,
                   float scale_z)
    : offset_x(ox), offset_y(oy), list(0), pVerts(0) {
  LoadObj(path, scale_x, scale_y, scale_z);
  InitVBO();
}

bool ObjModel::LoadObj(const char* pPathname,
                       float scale_x,
                       float scale_y,
                       float scale_z) {
  ...
  try {
    ...
    while (!fin.eof()) {
      ...
      // f v1/vt1/vn1 v2/vt2/vn2 v3/vt3/vn3
        ...
        Tfacet face;
        sscanf(buffer, "f %d/%d/%d %d/%d/%d %d/%d/%d", &face.v[0], &face.t[0],
               &face.n[0], &face.v[1], &face.t[1], &face.n[1], &face.v[2],
               &face.t[2], &face.n[2]);
        face -= 1;
    ...
    cout << "Totally " << vertices.size() << " vertices, " << txcoords.size()
         << " txcoords, " << norms.size() << " norms, " << faces.size()
         << " faces." << endl;
    ...
    for (int i = 0; i < faces.size(); i++) {  // go through the faces
      for (int j = 0; j < 3; j++) {           // three vertices per face
        ...
        if (faces[i].v[j] < vertices.size())
          ...
          cout << "face overflow : " << faces[i].v[j] << "  " << vertices.size()
               << endl;

        if (faces[i].t[j] < txcoords.size())
          cvt.t = txcoords[faces[i].t[j]];
        else
          cout << "txd overflow : " << faces[i].t[j] << "  " << txcoords.size()
               << endl;

        if (faces[i].n[j] < norms.size())
          cvt.n = norms[faces[i].n[j]];
        else
          cout << "norms overflow : " << faces[i].n[j] << "  " << norms.size()
               << endl;

        // use the same vertex if it is already in the list
        int repeatId = 0;
        for (; repeatId < cvertices.size() && !(cvt == cvertices[repeatId]);
             repeatId++)
          ;
...
void ObjModel::InitVBO() {
  ...
  try {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * n_Tri * 3 * 3, pVerts,
                 GL_STATIC_DRAW);
    printf("vertices,");

    glBindBuffer(GL_ARRAY_BUFFER, VBO[1]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * n_Tri * 3 * 3, pNorm,
                 GL_STATIC_DRAW);
    printf("norm,");

    glBindBuffer(GL_ARRAY_BUFFER, VBO[2]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * n_Tri * 3 * 2, pTxd,
                 GL_STATIC_DRAW);
    printf("txd,");

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, VBO[3]);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLushort) * n_Ind, pInd,
                 GL_STATIC_DRAW);
    printf("ind");
    printf("...Done!\n");
  } catch (...) {
