...
chrono::high_resolution_clock::time_point pclock =
...
void UpdateCameraPose() {
  ...
  camBack[0] = planePos[0] + plane_coord[8] * 5.0 -
               plane_coord[0] * (3.0 + throttleFactor * 2.0);
  camBack[1] = planePos[1] + plane_coord[9] * 5.0 -
               plane_coord[1] * (3.0 + throttleFactor * 2.0);
  camBack[2] = planePos[2] + plane_coord[10] * 5.0 -
               plane_coord[2] * (3.0 + throttleFactor * 2.0);
...
void RenderShadowDepth(M3DMatrix44f mvp, M3DMatrix44f mv) {
  ...
  glLoadIdentity();
  gluLookAt(planePos[0] + sun_target[0] * 5.0,
            planePos[1] + sun_target[1] * 5.0,
            planePos[2] + sun_target[2] * 5.0, planePos[0], planePos[1],
            planePos[2], 0.0, 0.0, 1.0);
...
void RenderScene(void) {
  ...
  glMatrixMode(GL_MODELVIEW);
  glLoadIdentity();
  gluLookAt(cameraPos[0], cameraPos[1], cameraPos[2], planePos[0], planePos[1],
            planePos[2], plane_coord[8], plane_coord[9], plane_coord[10]);
  ...
  if (isShowingF22) {
    lightingShader->use();
    lightingShader->setUniform("InvVP", UNI_MATRIX_4,
                               &inverse_model_view_proj[0]);
    lightingShader->setUniform("InvNVP", UNI_MATRIX_3,
                               &inverse_model_view_rot[0]);
    lightingShader->setUniform("ProjectionMatrix", UNI_MATRIX_4,
                               &projection[0]);
    lightingShader->setUniform("ShadowMVP", UNI_MATRIX_4,
                               &model_view_proj_shadow[0]);
    // lightingShader->setUniform("ShadowMV", UNI_MATRIX_4,
    // &model_view_shadow[0]);
  ...
  // pclock).count() / 1000000.0);
  pclock = chrono::high_resolution_clock::now();
}

void prepareFBO(GLenum textureNum,
                GLuint* fboname,
                GLuint* dbname,
                GLuint* texture,
                int width,
...
void SetupRC() {
  ...
  if (!GL_VERSION_1_3 && (!GL_ARB_multitexture || !GL_ARB_texture_cube_map)) {
    fprintf(stderr,
            "Neither OpenGL 1.3 nor necessary"
  ...
  glGenTextures(1, &texture);
  glBindTexture(GL_TEXTURE_2D, texture);
  glTexImage2D(GL_TEXTURE_2D, 0, component, width, height, 0, format,
               GL_UNSIGNED_BYTE, (void*)tga);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  ...
  prepareFBO(GL_TEXTURE0, &fbo_shadow, &shadow_buffer, &shadow_texture, 2048,
             2048, GL_DEPTH_COMPONENT24, GL_DEPTH_COMPONENT, GL_FLOAT);
